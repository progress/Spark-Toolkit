...
(function($, kendo, progress, window){
    ...
    if ($ && kendo && progress && typeof window.spark === "undefined") {
        ...
        window.spark = {
            ...
            version: "v3.3.0 (2017.12.15.053901)",
        ...
        window.spark.grid = {
            ...
            getSelectedRecord: function(event, grid){
                if (grid) {
                    ...
                    grid = event.sender; // The grid is the sender.
                    if (grid) {
            ...
            createDatePickerEditor: function(fieldOptions){
                return function(container, options){
                    ...
                    options.sparkEditor.grid = $(container).closest("div.k-grid").getKendoGrid();
                    setTimeout(function(){
            ...
            /**
             * Lazy-initializer for creating an editor that utilizes a resource to populate options.
             * @method createInvokeLookupEditor
             * @memberof spark.grid
             * @param {Object} fieldOptions Properties for widget
             * @returns {Object} Instance of a {@link spark.field.createInvokeLookup}
             */
            createResourceLookupEditor: function(fieldOptions){
                return function(container, options){
                    options.sparkEditor = window.spark.field.createResourceLookup($('<input data-bind="value:' + options.field + '"/>').appendTo(container), fieldOptions);
                    options.sparkEditor.grid = $(container).closest("div.k-grid").getKendoGrid();
                }
            }
