    ...
    //   (and when we remove this, remove the calls to it in this file)
        ...
        this._prependAppURL = function (oldURL) {
            ...
            if (!pat.test(newURL)) {
                ...
                // Add the serviceURI if value is non-null.
...
(function () {
    ...
    JSDOTransport = kendo.data.RemoteTransport.extend({
        ...
        _getModel: function () {
            ...
            var model = ((((transport.options || {}).dataSource || {}).options || {}).schema || {}).model || {};
            schema = transport.jsdo[transport.tableRef].getSchema();
            for (i = 0; i < schema.length; i += 1) {
                    ...
                    if (type === "array") {
                        for (j = 0; j < schema[i].maxItems; j += 1) {
                            ...
                            if (!transport.jsdo[transport.tableRef]._fields[element.name.toLowerCase()]) {
                                ...
                                // Merge catalog schema properties with code-supplied schema.
                                fields[element.name] = jQuery.extend(true, schema[i], (model.fields || {})[element.name] || {});
                                // Remove special properties from catalog schema.
                                delete fields[element.name].ablType;
                                delete fields[element.name]["default"];
                                fields[element.name].type = type;
                                // Only provide a defaultValue if none previously discovered/set.
                                if (fields[element.name].defaultValue === undefined && value !== undefined) {
                        ...
                        value = transport.jsdo._getDefaultValue(schema[i]);
                        // Merge catalog schema properties with code-supplied schema.
                        fields[schema[i].name] = jQuery.extend(true, schema[i], (model.fields || {})[schema[i].name] || {});
                        // Remove special properties from catalog schema.
                        delete fields[schema[i].name].ablType;
                        delete fields[schema[i].name]["default"];
                        fields[schema[i].name].type = type;
                        // Only provide a defaultValue if none previously discovered/set.
                        if (fields[schema[i].name].defaultValue === undefined && value !== undefined) {
